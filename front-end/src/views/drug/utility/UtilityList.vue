<template>
    <div>
        <v-btn class="mx-0 mt-0 mb-2 text-capitalize" color="primary" small dark
               ...
               @click="utilityCreateMethod({})">
            <v-icon class="mr-1">add_circle_outline</v-icon>
            Add Drug Utility
        </v-btn>
        <div v-if="utilities.length > 0">
            <v-layout row wrap>
                <v-flex xs12 sm6 v-for="item in utilities" :key="item.id">
                    <v-card>
                        <v-toolbar class="primary--text" height="30" flat dense card>
                            ...
                            <v-spacer/>
                            <v-icon small class="mr-2" @click="utilityCreateMethod(item)">edit</v-icon>
                            <v-icon small @click="utilityDeleteMethod(item)">delete</v-icon>
                        </v-toolbar>
                        <v-divider/>
                        <v-card-text>
        ...
        <div v-else>
            <v-card class="mt-2 mb-3 pa-4">
                <span class="body-1 text-xs-center">There is nothing to show. Please add by pressing <b>Add Drug Utility</b> button above.</span>
            </v-card>
        </div>
    </div>
</template>
...
<script>
    ...
    import DrugUtilityCreate from "./UtilityCreate"
    ...
    export default {
        ...
        methods: {
            ...
            utilityDeleteMethod(utility) {
                const where = {drugUtilityId: utility.id};
                DrugAPI.count({where: where}).then((res) => {
                    if (res.count == 0) {
                        ...
                        eventBus.$emit('openSb', {message: 'Drug utility is not delete safe!', color: "info"});
                    }
                });
            },
            ...mapActions(['fetchUtilities', 'deleteUtility'])
        },
        created() {
            this.fetchUtilities();
        },
        computed: mapGetters(['utilities']),
        mounted() {
